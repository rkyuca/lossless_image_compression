...
def linearize_hilbert_curve_back(img) :
    ...
    dmax=n*n
    #print('linearize_hilbert_curve:  w:%d h:%d n:%d dmax:%d'%(w,h,n,dmax))
    #import ipdb;ipdb.set_trace()
    for i in range(c):
        for d in range(dmax):
            x,y=hc.d2xy(n,d)
            if x>=w or y>=h:
                continue    
            img_lin.append(img[i][x][y])
    #print('img.shape: '+str(img.shape)+' pixels: '+str(c*w*h)+' dmax: '+str(dmax)+' len(img_lin): '+str(len(img_lin)))
    return img_lin

def linearize_hilbert_curve(img) :
    c,w,h=img.shape
    img_lin=[]    
    whmax=max(w,h)
    n=int(2**np.ceil(np.log2(max(w,h)))) #next 2th power of max dimension if not power of 2
